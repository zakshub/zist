...
export default function Dashboard() {
  return (
    <main>
      ...
      <section className="grid">{stages.map(([name, detail, state], index) => <article key={name}><span className="number">0{index + 1}</span><div><h3>{name}</h3><p>{detail}</p></div>{index < 2 ? <Link className="action" href={index === 0 ? '/content-memory' : '/topics'}>Explore</Link> : <span className={`dot ${state}`} aria-label={state} />}</article>)}</section>
      <footer><span>Phase 5 · Topic Engine</span><span>Asia/Karachi</span></footer>
    </main>
  );
}
