...
const stages = [
  ...
  ['Analytics', 'Synthetic observations', 'safe'],
] as const;
...
export default function Dashboard() {
  return (
    <main>
      ...
      <section className="grid">{stages.map(([name, detail], index) => <article key={name}><span className="number">0{index + 1}</span><div><h3>{name}</h3><p>{detail}</p></div><Link className="action" href={index === 0 ? '/content-memory' : index === 1 ? '/topics' : index === 2 ? '/articles' : index === 3 ? '/images' : index === 4 ? '/blogger' : index === 5 ? '/facebook' : index === 6 ? '/schedule' : '/analytics'}>Explore</Link></article>)}</section>
      <footer><span>Phase 12 · Analytics</span><span>Asia/Karachi</span></footer>
    </main>
  );
}
