...
export default function Dashboard() {
  return (
    <main>
      ...
      <section className="grid">{stages.map(([name, detail, state], index) => <article key={name}><span className="number">0{index + 1}</span><div><h3>{name}</h3><p>{detail}</p></div>{index === 0 ? <Link className="action" href="/content-memory">Explore</Link> : <span className={`dot ${state}`} aria-label={state} />}</article>)}</section>
      <footer><span>Phase 2 · Data Model + Mock Archive</span><span>Asia/Karachi</span></footer>
    </main>
  );
}
