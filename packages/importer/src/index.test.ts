...
it('imports and duplicate-checks 7,000 synthetic Urdu records',()=>{const database=new DatabaseSync(':memory:');migrate(database);const repository=new SourcePostRepository(database);const records=Array.from({length:7000},(_,index)=>({externalId:`scale-${index}`,source:'SCALE_TEST',originalText:`یہ مصنوعی اردو تحریر نمبر ${index} ہے۔`}));const content=JSON.stringify(records);const first=importArchive(content,'scale.json',repository);expect(first).toMatchObject({total:7000,imported:7000,duplicates:0,invalid:0});const replay=importArchive(content,'scale.json',repository);expect(replay).toMatchObject({total:7000,imported:0,duplicates:7000,invalid:0});expect(repository.summary().total).toBe(7000);database.close()});
